...
public class FsRepository implements Repository {
    ...
    @VisibleForTesting
    static final String ALL_FILE_INHERIT_ACL = "allFilesAcl";
    @VisibleForTesting
    static final String CHILD_FOLDER_INHERIT_ACL = "childFoldersAcl";
    @VisibleForTesting
...
if (!filePatterns.isInclude(docName))
{
    ...
    private void getDirectoryContent(
        ...
        try (DirectoryStream<Path> files = delegate.newDirectoryStream(doc)) {
            ...
            for (Path file : files) {
              ...
              /*  //SADA Changes to exclude
                String sAbsolutePath = file.toString();
                log.log(Level.INFO, "Checking for include/exclude for ::[" + sAbsolutePath + "]");
                if ((Files.exists(file)) && (Files.isDirectory(file))) {
                    //Check if it matches the exclude patterns

                    if ((fileExclusion.IsExcluded(sAbsolutePath)) && (isExclude)) {
                        log.log(Level.INFO, "File Excluded for  ::[" + sAbsolutePath + "]");

                        continue;
                    }
                    else if ((!fileExclusion.IsExcluded(sAbsolutePath)) && (!isExclude)) {
                        log.log(Level.INFO, "File Excluded for  ::[" + sAbsolutePath + "]");

                        continue;
                    }
                }
                log.log(Level.INFO, "File Included for ::[" + file + "]"); */
    ...
    private void getFileContent(Path doc, FileTime lastAccessTime, Item item,
        ...
        try {
            ...
            log.log(Level.INFO, "ERROR Getting Mime Type for ::[" + doc + "]");
            //set default
            mimeType = "application/octet-stream";
            item.getMetadata().setMimeType(mimeType);
        }
        ...
        log.log(Level.INFO, "Getting content for::[" + doc + "] with Mime::[" + mimeType + "]");
        ...
        if (entityRecognition != null) {
            if (docFile.length() <= maxFileSizeBytesToParse) {
                ...
                try {
                    ...
                    log.log(Level.INFO, "Processing entities for Mime Type (" + mimeType + ") : ");
                    try {
                    ...
                    log.log(Level.INFO, "Found entities for file (" + doc.toString() + ") : " + entities);
